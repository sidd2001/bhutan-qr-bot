...
async function scanAndAddPassenger(ctx, userId, presetNum, fileId, withAutoConfirm) {
  try {
    ...
    if (!passengers.length) {
      return ctx.reply(
        "⚠️ Couldn't read ID from photo. Try:\n• A clearer photo\n• Better lighting\n• Or tap ⌨️ Type ID manually",
        Markup.inlineKeyboard([
...
async function runPreset(ctx, num, passengers, testMode) {
  ...
  const paxLine = passengers.length ? `👥  ${passengers.length} passenger${passengers.length > 1 ? "s" : ""}` : `🧍  Solo trip`;
...
bot.on("text", async (ctx) => {
  ...
  if (wiz) {
    ...
    if (wiz.step === "add_passenger_manual") {
      ...
      // Auto-save & add to current QR
      const p = Store.addOrGetPassenger(userId, {
        name: `Passenger ${docNumber.slice(-4)}`,
        docType: "Passport",
        docNumber,
        nationality: "Indian",
      });
      Store.clearWizard(userId);
      await ctx.reply(`✅ Passenger *${p.name}* saved (ID: ${docNumber})\n\nGenerating QR...`, { parse_mode: "Markdown" });
