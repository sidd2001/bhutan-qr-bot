...
async function runPreset(ctx, num, passengers, testMode) {
  ...
  // Build detailed passenger line showing their names
  let paxLine;
  let paxDetails = "";
  if (passengers.length === 0) {
    ...
    paxDetails = `\n*Passenger:*\n• ${passengers[0].name}  _(${passengers[0].docNumber})_`;
  } else {
    paxLine = `👥  With ${passengers.length} passengers`;
    paxDetails = `\n*Passengers:*\n` + passengers.map(p => `• ${p.name}  _(${p.docNumber})_`).join("\n");
  }
  ...
  try {
    ...
    // Build passenger list for final QR caption
